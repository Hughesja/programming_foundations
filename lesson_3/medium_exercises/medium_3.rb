# Question 1: Every named entity in Ruby has an object_id. This is a unique identifier for that object.

# It is often the case that two different things "look the same", but they can
# be different objects. The "under the hood" object referred to by
# a particular named-variable can change depending on what is done to that named-variable.
...
# Question 2: Let's look at object id's again from the perspective of a method call instead of a block.

# Here we haven't changed ANY of the code outside of inside of the block/method.
# We simply took the contents of the block from the previous exercise and moved
...
# parameters and see how even though they are treated in the same way by
...
puts "My string looks like this now: #{my_string}" # pumpkinks
puts "My array looks like this now: #{my_array}" # [pumpkins, rutabaga]


# Question 4: To drive that last one home...let's turn the tables and have the
# string show a modified output, while the array thwarts the method's efforts
